...
{
    return int_to_bool(get_pawn_value(g, is_white, selectedPawn, ALIVE)) && !int_to_bool(get_pawn_value(g, is_white, selectedPawn, QUEEN)) && get_pawn_value(g, is_white, selectedPawn, ENNEMY) == VOID_INDEX && !has_friend(g, selectedPawn, is_white) && get_pawn_value(g, is_white, selectedPawn, PBA) == 1;
}
